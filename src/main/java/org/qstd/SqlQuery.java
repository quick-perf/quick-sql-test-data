...
public class SqlQuery {
  ...
  /**
   * Constructor to instantiate an SQL query from an SQL query with bind parameters and its bind
   * parameter values
   *
  ...
  static SqlQuery buildFromRow(
      Collection<String> columnNamesToSearch, DatasetRow rowToSearch, DatabaseType dbType) {
    Map<String, Object> valuesToMatch = rowToSearch.getColumnValueByColumnName();
    String whereConditions =
        valuesToMatch.entrySet().stream()
            .map(
                entry -> {
                  ...
                  return columnName
                      + (entry.getValue() == null
                          ? " IS NULL"
                          : "="
                              + ColumnValueFormatter.INSTANCE.formatColumnValue(
                                  entry.getValue(), dbType));
                })
            .collect(joining(" AND "));
