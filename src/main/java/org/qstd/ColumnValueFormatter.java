...
class ColumnValueFormatter {
  ...
  String formatColumnValue(Object columnValue, DatabaseType dbType) {
    if (columnValue == null) {
      return "NULL";
    } else if (DatabaseType.ORACLE.equals(dbType) && columnValue instanceof Timestamp) {
      Timestamp timeStamp = (Timestamp) columnValue;
      return buildOracleToDateFunctionFor(timeStamp);
    } else if (DatabaseType.ORACLE.equals(dbType) && isOracleSqlTimestamp(columnValue)) {
      return buildOracleToTimeStampFunctionFor(columnValue);
    } else if (DatabaseType.HSQLDB.equals(dbType) && columnValue instanceof OffsetDateTime) {
  ...
  private String buildOracleToTimeStampFunctionFor(Object columnValue) {
    ...
    boolean dateHasMsLessThan100 =
        oracleTimeStampAsString.length() == aDateWithMsLessThan100.length();
    String dateForTimeStampCreation =
        dateHasMsLessThan100 ? oracleTimeStampAsString + "0" : oracleTimeStampAsString;
