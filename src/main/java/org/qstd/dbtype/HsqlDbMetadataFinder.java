...
class HsqlDbMetadataFinder implements DatabaseMetadataFinder {
  ...
  private static final SqlQuery HSQL_DB_REFERENCED_TABLES_QUERY =
      new SqlQuery(
          "with\n"
  ...
  private static final SqlQuery HSQL_DB_COLUMNS_MAPPINGS_QUERY =
      new SqlQuery(
          "select\n"
  ...
  HsqlDbMetadataFinder(DataSource dataSource) {
    ...
    this.hsqlDbReferencedTablesFinder =
        new BaseReferencedTablesFinder(dataSource, HSQL_DB_REFERENCED_TABLES_QUERY);
    this.hsqlDbColumnsMappingsFinder =
