...
public class DatabaseMetadataFinderWithCache implements DatabaseMetadataFinder {
  ...
  private final ConcurrentHashMap<String, Collection<String>> notNullColumnsByTableName =
      new ConcurrentHashMap<>();

  private final ConcurrentHashMap<String, List<String>> databaseColumnOrdersByTableName =
      new ConcurrentHashMap<>();

  private final ConcurrentHashMap<String, ColumnsMappingGroup> columnsMappingsByTableName =
      new ConcurrentHashMap<>();

  private final ConcurrentHashMap<String, ReferencedTableSet> referencedTableSetByTableName =
      new ConcurrentHashMap<>();

  private final ConcurrentHashMap<String, List<String>> primaryColumnsByTableName =
      new ConcurrentHashMap<>();
  ...
  public List<String> findDatabaseColumnOrdersOf(String tableName) {
    return databaseColumnOrdersByTableName.computeIfAbsent(
        tableName, t -> delegate.findDatabaseColumnOrdersOf(tableName));
  }

  @Override
  public ColumnsMappingGroup findColumnsMappingsOf(String tableName) {
    return columnsMappingsByTableName.computeIfAbsent(
        tableName, t -> delegate.findColumnsMappingsOf(tableName));
  }

  @Override
  public Collection<String> findNotNullColumnsOf(String tableName) {
    return notNullColumnsByTableName.computeIfAbsent(
        tableName, t -> delegate.findNotNullColumnsOf(tableName));
  }

  @Override
  public ReferencedTableSet findReferencedTablesOf(String tableName) {
    return referencedTableSetByTableName.computeIfAbsent(
        tableName, t -> delegate.findReferencedTablesOf(tableName));
  }

  @Override
  public List<String> findPrimaryColumnsOf(String tableName) {
    return primaryColumnsByTableName.computeIfAbsent(
        tableName, t -> delegate.findPrimaryColumnsOf(tableName));
  }
