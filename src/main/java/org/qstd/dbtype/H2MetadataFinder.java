...
class H2MetadataFinder implements DatabaseMetadataFinder {
  ...
  private static final SqlQuery H2_REFERENCED_TABLES_QUERY =
      new SqlQuery(
          "with\n"
  ...
  private static final SqlQuery H2_COLUMNS_MAPPINGS_QUERY =
      new SqlQuery(
          "select \n"
  ...
  H2MetadataFinder(DataSource dataSource) {
    ...
    this.h2ReferencedTablesFinder =
        new BaseReferencedTablesFinder(dataSource, H2_REFERENCED_TABLES_QUERY);
    this.h2ColumnsMappingsFinder =
        new BaseColumnsMappingsFinder(dataSource, H2_COLUMNS_MAPPINGS_QUERY);
