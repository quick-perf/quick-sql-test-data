...
/**
 * Class allowing to ease the generation of datasets to test SQL queries. Methods produce INSERT
 * statements taking account of database integrity constraints.
 *
 * @see SqlQuery
 * @see DatasetRow
...
public class QuickSqlTestData {
  ...
  /**
   * Factory method to build an instance of <code>org.qstd.QuickSqlTestData</code> from a data
   * source. <em>The retrieval of database metadata, such as not null columns or primary keys, is
   * cached for each table.</em>
   *
  ...
  public static QuickSqlTestData buildFrom(DataSource dataSource) {
    ...
    DatabaseMetadataFinder databaseMetadataFinder =
        DatabaseMetadataFinderFactory.createDatabaseMetadataFinderFrom(dataSource, dbType);
    DatabaseMetadataFinder databaseMetadataFinderWithCache =
        DatabaseMetadataFinderWithCache.buildFrom(databaseMetadataFinder);
    return buildFrom(dataSource, dbType, databaseMetadataFinderWithCache);
  }

  /**
   * Factory method to build an instance of <code>org.qstd.QuickSqlTestData</code> from a data
   * source, a database type and a database metadata finder.
   *
   ...
   * @return An instance of <code>org.qstd.QuickSqlTestData</code>
   */
  public static QuickSqlTestData buildFrom(
      DataSource dataSource, DatabaseType dbType, DatabaseMetadataFinder databaseMetadataFinder) {
    DatasetRowsGenerator datasetRowsGenerator =
        new DatasetRowsGenerator(dataSource, dbType, databaseMetadataFinder);
    return new QuickSqlTestData(datasetRowsGenerator, dbType);
  }

  /**
   * Generates an SQL script allowing to test the SQL query given in parameter. This script contains
   * INSERT statements. It takes into account the database integrity constraints.
   *
  ...
  /**
   * Generates an SQL script allowing to test an SQL query with its bind parameter values. This
   * script contains INSERT statements. It takes into account the database integrity constraints.
   *
  ...
  /**
   * Generates an SQL script allowing to test the list of SQL queries given in parameter. This
   * script contains INSERT statements. It takes into account the database integrity constraints.
   *
  ...
  /**
   * Generates a list of INSERT statements allowing to test the list of SQL queries given in
  ...
  public String generateInsertScriptFor(String... sqlQueries) {
    List<SqlQuery> queries = stream(sqlQueries).map(SqlQuery::new).collect(toList());
   ...
   * @return A list of INSERT statements allowing to create in database the dataset row given in
   *     parameter
   */
  ...
  public List<String> generateInsertListFor(String... sqlQueries) {
    List<SqlQuery> sqlQueryObjects = stream(sqlQueries).map(SqlQuery::new).collect(toList());
