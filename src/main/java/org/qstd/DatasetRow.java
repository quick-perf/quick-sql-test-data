...
public class DatasetRow {
  ...
  void sortColumnsFollowing(List<String> databaseColumnOrders) {
    if (!databaseColumnOrders.isEmpty()) {
      ColumnNamesComparator columnNamesComparator =
          ColumnNamesComparator.from(databaseColumnOrders);
  ...
  Collection<DatasetRow> extractJoinedRowsFrom(ColumnsMappingGroup columnsMappingGroup) {
    return columnValueByColumnName.entrySet().stream()
        .map(
            valueForColumn -> {
  ...
  private Optional<DatasetRow> buildOptionalRowFrom(
      Map.Entry<String, Object> valueForColumn,
      Optional<ColumnMappingPart> optionalMappingForColumn) {
    return optionalMappingForColumn.map(
        columnMappingPart -> {
